...
using Fiddler.Properties;
using Fiddler.Controls.Plugin;

namespace Fiddler.Forms
{
    public partial class MainForm : Form
    {
        ...
        public MainForm(ILogger<MainForm> logger)
        {
            ...
            if (AppSettings.DarkMode)
            {
                ...
                StartTab.BackgroundImage = Resources.Fiddler_bk;
            ...
            TabPanel.ControlAdded += TabPanel_TabsChanged;
            TabPanel.ControlRemoved += TabPanel_TabsChanged;
        ...
        private void TabPanel_TabsChanged(object sender, ControlEventArgs e)
        {
            // Ignore the doorbell while WE are the ones rearranging the tabs.
            if (_isSortingTabs)
            {
                return;
            }

            // Only defer with BeginInvoke once the window actually exists.
            if (IsHandleCreated)
            {
                BeginInvoke(new Action(SortTabsAlphabetically));
            }
            else
            {
                SortTabsAlphabetically();
        ...
        // This little "flag" stops the sorter from calling itself over and over.
        private bool _isSortingTabs = false;

        private void SortTabsAlphabetically()
        {
            // If we're already sorting, don't start again (prevents an endless loop).
            if (_isSortingTabs)
            {
                return;
            }
            ...
            try
            {
                // Remember which tab the user was looking at, so we can re-select it after.
                TabPage previouslySelected = TabPanel.SelectedTab;

                // Take all the tabs off the shelf into a temporary list.
                var pages = TabPanel.TabPages.Cast<TabPage>().ToList();

                // Put them back in order:
                //   - Keep the "StartTab" (welcome tab) first.
                //   - Then everything else sorted A -> Z by its Text (ignoring caps).
                var ordered = pages
                    .OrderBy(p => p == StartTab ? 0 : 1)
                    .ThenBy(p => p.Text, StringComparer.OrdinalIgnoreCase)
                    .ToList();

                // Rebuild the shelf in the new order.
                TabPanel.SuspendLayout();
                TabPanel.TabPages.Clear();
                foreach (TabPage page in ordered)
                {
                ...
                if (previouslySelected != null && TabPanel.TabPages.Contains(previouslySelected))
                {
                    TabPanel.SelectedTab = previouslySelected;
                }
            }
            finally
            {
                // We're done sorting; allow future sorts again.
                _isSortingTabs = false;
            }
        }
    }
}